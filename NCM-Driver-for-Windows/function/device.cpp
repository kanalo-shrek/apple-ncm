...
#define PENDING_BULK_IN_READS   (8)  // Increased from 3 for high-bandwidth streaming
...
)
{
    ...
    if (ExAcquireRundownProtection(&functionDevice->m_BulkInRundown) != FALSE)
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            WDF_REQUEST_SEND_OPTIONS_SET_TIMEOUT(&sendOptions, WDF_REL_TIMEOUT_IN_SEC(5));  // Match host timeout
